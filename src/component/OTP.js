...
const OTPView = ({ page, mobile_number, onSuccess }) => {
  const {
    sendRequest: fetch_user,
    sendRequest: resend_otp,
    isLoading,
    error,
  } = useHttp();
          ...
          resend_otp(
            {
              ...
              body: {
                mobile_number:
                  response.data.country_code.toString() +
      ...
      <div className="otp-container" id="otp-container">
        <div className="login-container">
          ...
          <div className="login-title">
            Inserisci il codice che ti è arrivato per messaggio
          </div>
          <input
            type="text"
            id={"otp-" + page}
          ...
          />
          <button className="login-button" onClick={check_otp}>
            Convalida & Conferma
          </button>
        </div>
      </div>
