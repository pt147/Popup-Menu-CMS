...
import { SUBEVENTDETAIL, SUBEVENTTICKETS, FETCH_COLOR_SETTINGS, GET_METHOD } from "../apis/apiHelper";
import useHttp from "../hooks/use-http";
import { useParams } from "react-router";

function SubEventBooking() {
    const { sendRequest: getSubEventDetailsApi, sendRequest: getSubEventTicketsApi, sendRequest: fetchColorSettingsAPI, isLoading, error } = useHttp();
    ...
    const updateAmount = (ticket_id, is_add) => {
        ...
        if (is_add && my_ticket.ticket.quantity > my_ticket.amount && 10 > my_ticket.amount){
            my_ticket.amount++;
    ...
    return (
        <div className="subevents-container">
            ...
            {
                tickets && to_buy_tickets &&
                to_buy_tickets.map(ticket => {
                    return (
                        <div className="subevent-booking-ticket-container" key={ticket.ticket.id}>
                            ...
                            <div className="subevent-booking-ticketbooking-container">
                                <div className="subevent-booking-ticketbooking-counter">
                                    <div className="subevent-booking-ticketbooking-button-container">
                                        <div className="subevent-booking-ticketbooking-button" onClick={() => updateAmount(ticket.ticket.id, false)}>
                                            -
                                        </div>
                                    </div>
                                    <div className="subevent-booking-ticketbooking-count" id={ticket.ticket.id}>
                                        {ticket.amount}
                                    </div>
                                    <div className="subevent-booking-ticketbooking-button-container">
                                        <div className="subevent-booking-ticketbooking-button" onClick={() => updateAmount(ticket.ticket.id, true)}>
                                            +
                                        </div>
                                    </div>
                                </div>
                                {
                                    /*
                                    <div className="subevent-booking-ticketbooking-available">
                                        {ticket.ticket.quantity > 0 ? "disponibili: " + ticket.ticket.quantity : 'SOLD OUT'}
                                    </div>
                                    */
                                }
                            </div>
