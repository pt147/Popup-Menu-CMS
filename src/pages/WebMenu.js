  ...
  const geteventDetails = () => {
    ...
    fetchColorSettingsAPI(
      ...
      (response) => {
        ...
        document.getElementById("nav-content").style.backgroundColor =
          response.data.bgMenuColor;
  ...
  useEffect(() => {
    getMenuApi(
      ...
      (response) => {
        ...
        const most_items = all_items.filter(
          (item) => item.is_available === true
        );
        const items = most_items.filter((item) =>
        ...
        items.forEach((item) => {
          if (item.category_id === null && item.sub_menu_ids.length === 0) {
            ...
            if (
              !noParentCategories.some(
                (noParentCategory) => noParentCategory.id === item.category_id
              )
            ) {
        ...
        noParentCategories.forEach((noParentCategory) => {
          ...
          items.forEach((item) => {
            if (
              item.sub_menu_ids.length === 0 &&
              item.category_id === noParentCategory.id
            ) {
        ...
        submenus.forEach((submenu) => {
          ...
          items.forEach((item) => {
            if (
              item.sub_menu_ids.some((submenu_id) => submenu_id === submenu.id)
            ) {
              if (item.category_id === null) {
                my_items[i].push(item);
              } else {
                if (
                  !my_categories[i].some(
                    (my_category) => my_category.id === item.category_id
                  )
                ) {
          ...
          my_categories[i].forEach((category) => {
            ...
            items.forEach((item) => {
              if (
                ...
                item.sub_menu_ids.some(
                  (submenu_id) => submenu_id === submenu.id
                )
              ) {
  ...
  function leftClick(item) {
    ...
    mySelectedItems.forEach((item) => {
      ...
      orderWithdrawTypes = orderWithdrawTypes.filter((orderWithdrawType) =>
        item.orderWithdrawTypes.includes(orderWithdrawType)
      );
    });
    var my_item = mySelectedItems.find((my_item) => my_item.id === item.id);
    if (
      submenu_id
        ? item.parentSubmenu.id === submenu_id
        : true &&
          haveCommonElements(orderWithdrawTypes, item.orderWithdrawTypes)
    ) {
      if (my_item) {
      ...
      document.getElementById("banner-quantity-text-" + item.id).value =
        my_item.quantity;
      checkButtonEnable();
    }
  }
  ...
  function rightClick(item) {
    ...
    if (submenu_id ? item.parentSubmenu.id === submenu_id : true) {
      if (haveCommonElements(orderWithdrawTypes, item.orderWithdrawTypes)) {
        ...
        alert(
          "Non puoi aggiungere questo prodotto perché è incompatibile con la modalità di consegna degli altri prodotti selezionati"
        );
      }
    } else {
      alert(
        "Non puoi aggiungere questo prodotto perché appartiene ad un altro menù rispetto agli altri prodotti selezionati"
      );
    }
  }
            ...
            <Col xl={6}>
              <Card className="curved border-0 " id="card">
                <Card.Body className="p-4">
                  <center>
                    {event?.images.find((image) => image.banner_url)
                      .banner_url && (
                      <img
            ...
            <div className="menu_container">
              ...
              {menu &&
                menu.categories.map((category) => {
                  return (
                    <div key={category.category.name} className="menu_row">
                      <CategoryView
                        selectedItems={selectedItems}
                ...
                >
                  <Modal.Body>
                    <div className="item_container">
                      ...
                      />
                      <div className="item_title">{currentItem?.name}</div>
                      <div class="x_button" onClick={() => setModal(false)}>
                        ×
                      </div>
                      <div className="item_price_container">
                        <div className="item_price">{currentItem?.price} €</div>
                      </div>
