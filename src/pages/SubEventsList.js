...
import stats_image from "../images/stats.png";

function SubEventsList() {
    const { sendRequest: getEventDetailsApi, sendRequest: fetchColorSettingsAPI, isLoading, error } = useHttp();
                        ...
                        <div className="subevent-container" key={subevent.subEventId}>
                            <a href={"/Biglietteria/" + event.id + "-" + subevent.subEventId}>
                                <div className="subevent-topinfo-container">
                                    <img src={subevent.images[0].preview_url} className="subevent-image" />
                                    <div className="subevent-maininfo-container">
                                        <div className="subevent-title">
                                            {subevent.subEventName}
                                        </div>
                                        <div className="subevent-time">
                                            {moment(subevent.subEventStartDate).format('DD MMM, YYYY')} <br />
                                            {moment(subevent.subEventFromTime, "HH:mm:ss").format('HH:mm')} - {moment(subevent.subEventToTime, "HH:mm:ss").format('HH:mm')}
                                        </div>
                                    </div>
                                ...
                                <div className="subevent-otherinfo-container">
                                    <div className="subevent-price">
                                        Clicca per Partecipare
                                    </div>
                                    {/*<div className="subevent-likes">
                                        <img src={heart_image} className="subevent-litteicon"/> 
                                        1234
                                    </div>*/}
                                    {/*<div className="subevent-guestscount">
                                        <img src={stats_image} className="subevent-litteicon"/> 
                                        {subevent.subEventGuestCount}
                                    </div>*/}
                                </div>
                            </a>
                        </div>
