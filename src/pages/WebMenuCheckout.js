...
function WebMenuCheckout() {
  const {
    ...
    sendRequest: getMenuApi,
    sendRequest: addUserOrderApi,
    sendRequest: getClientSecretKeyApi,
    sendRequest: createCostumerApi,
  ...
  function getWithdrawalTypes() {
    ...
    items.forEach((item) => {
      orderWithdrawTypes = orderWithdrawTypes.filter((orderWithdrawType) =>
        item.item.orderWithdrawTypes.includes(orderWithdrawType)
      );
    });
  ...
  function attendEvent(isCash) {
    ...
    const instructions = document.getElementById(
      "menu-checkout-instructions"
    ).value;
    if (withdrawalTypes.length > 1) {
      const radios = document.getElementsByClassName(
        "menu-checkout-withdrawal"
      );
      for (let i = 0; i < radios.length; i++) {
        if ((radios[i].type = "radio")) {
          if (radios[i].checked) withdrawalType = radios[i].id;
        }
      }
    } else {
    ...
    if (withdrawalType === "serveAtTable") {
      tableNumber = document.getElementById(
        "menu-checkout-withdrawal-table"
      ).value;
    }
    const user_orders = [];
    items.forEach((item) => {
  ...
  return (
    <div style={{ width: "100%", height: "100%", marginBottom: "150px" }}>
      <div
        className="subevent-booking-checkout-container"
        id="normal-container"
      >
        <div className="subevent-booking-checkout-event-container">
          <img
            src={event && event.images[0].banner_url}
            className="subevent-booking-checkout-event-image"
          />
        ...
        <div className="subevent-booking-checkout-descriptions-container">
          <div className="subevent-booking-checkout-subevent-description">
            {subevent &&
              moment(subevent.subEventStartDate).format("DD MM YYYY")}{" "}
            -{" "}
            {subevent &&
            ...
            -{" "}
            {subevent &&
              moment(subevent.subEventToTime, "HH:mm:ss").format("HH:mm")}
          </div>
        ...
        <div className="subevent-booking-checkout-tickets-container">
          {items &&
            items.map((item) => {
              return (
                <div
                  className="subevent-booking-checkout-ticket-container"
          ...
          <div className="subevent-booking-checkout-ticket-container">
            <div
              className="subevent-booking-checkout-ticket-title"
              style={{ fontWeight: "500" }}
            >
              Totale
            </div>
            <div
              className="subevent-booking-checkout-ticket-amount"
              style={{ fontWeight: "500" }}
            >
                    ...
                    return (
                      <div>
                        <input
                          type="radio"
                          id="serveAtTable"
                          className="menu-checkout-withdrawal"
                          name="fieldset"
                        />
                        <label className="menu-checkout-withdrawal-label">
                          Servizio al Tavolo
                        </label>
                        <input
                          type="text"
                          placeholder="N° Tavolo"
                          className="menu-checkout-withdrawal-label"
                          id="menu-checkout-withdrawal-table"
                        />
                      </div>
                    );
                  case "counter":
        ...
        <div className="menu-checkout-withdrawal-container">
          <textarea
            className="menu-checkout-instructions"
            id="menu-checkout-instructions"
            placeholder="Aggiungi delle istruzioni..."
          ></textarea>
        </div>
        ...
        {totalPrice > 0 && (
          <div>
            {paymentMethods.find((pm) => pm === "Cash") && (
              <>
                <button
                  className="subevent-booking-checkout-confirm-button subevent-booking-checkout-confirm-middable-button"
                  onClick={() => prepare_order(true)}
                >
            ...
            {paymentMethods.find((pm) => pm === "Digital") && (
              <div>
                <button
                  className="subevent-booking-checkout-confirm-button subevent-booking-checkout-confirm-middable-button"
                  onClick={() => prepare_order(false)}
                >
                  {isPaymentProcessing
                    ? "Pagamento in corso..."
                    : "Conferma e paga con Carta"}
                </button>
        ...
        {totalPrice === 0 && (
          <div>
            <button
              className="subevent-booking-checkout-confirm-button"
              onClick={() => prepare_order(true)}
            >
              Conferma
            </button>
      ...
      <LoginView isOTPRequired={false} onSuccess={helper} />
